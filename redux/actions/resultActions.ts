import { ACTIVITY_ID_TEST_ID, SET_ALL_ABOUT, SET_RESULT } from "./../types";
...
export const getResult = () => (dispatch: any) => {
  axiosPost(
    APIPath.results,
    (error) => {
    ...
    (value: []) => {
      value.forEach((result: { activate_id: number }) => {
        dispatch(getAboutActivate(result.activate_id));
      });
      dispatch({
        type: SET_RESULT,
        payload: value,
...
export const getAboutActivate = (activate_id: number) => (dispatch: any) => {
  axiosPost(
    APIPath.getActivate,
    (error) => {
      console.log("Error: " + error);
    ...
    (value) => {
      dispatch(getAboutTest(value.testing_id));
      dispatch({
        type: ACTIVITY_ID_TEST_ID,
        payload: {
          testId: value.testing_id,
          activityId: activate_id,
        },
      });
    },
    {
